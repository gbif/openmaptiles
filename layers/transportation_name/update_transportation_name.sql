...
CREATE MATERIALIZED VIEW osm_transportation_name_linestring_gen1 AS (
    SELECT ST_Simplify(geometry, 4.0e-9) AS geometry, osm_id, name, name_en, name_de, tags, ref, highway, network, z_order
    FROM osm_transportation_name_linestring
    WHERE highway IN ('motorway','trunk')  AND ST_Length(geometry) > 6.5e-7
);
...
CREATE MATERIALIZED VIEW osm_transportation_name_linestring_gen2 AS (
    SELECT ST_Simplify(geometry, 9.7e-9) AS geometry, osm_id, name, name_en, name_de, tags, ref, highway, network, z_order
    FROM osm_transportation_name_linestring_gen1
    WHERE highway IN ('motorway','trunk')  AND ST_Length(geometry) > 1.1e-6
);
...
CREATE MATERIALIZED VIEW osm_transportation_name_linestring_gen3 AS (
    ...
    WHERE highway = 'motorway' AND ST_Length(geometry) > 1.6e-6
);
...
CREATE MATERIALIZED VIEW osm_transportation_name_linestring_gen4 AS (
    SELECT ST_Simplify(geometry, 4.0e-8) AS geometry, osm_id, name, name_en, name_de, tags, ref, highway, network, z_order
